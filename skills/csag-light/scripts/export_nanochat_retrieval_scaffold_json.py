...
def quote_bank(artifact: dict, max_lines=12, max_quote_chars=120):
    lines = []

    def add_items(field_name, id_key):
        for item in artifact.get(field_name, []):
            provenance = item.get("provenance", [])
            if provenance and isinstance(provenance, list):
                quote = provenance[0].get("quote", "")
                if quote:
                    short = quote[:max_quote_chars].strip()
                    lines.append(f'{item[id_key]}: "{short}"')

    add_items("entities", "entity_id")
    add_items("hypotheses", "hypothesis_id")
    add_items("evidence_items", "evidence_id")
    add_items("discoveries", "discovery_id")
    add_items("gaps", "gap_id")
    add_items("conclusions", "conclusion_id")
    return "\n".join(lines[:max_lines])


def retrieval_scaffold_prompt(title: str, abstract: str, scaffold: dict, artifact: dict) -> str:
    scaffold_json = json.dumps(scaffold, ensure_ascii=True, separators=(",", ":"))
    bank = quote_bank(artifact)
    return (
        f"Paper title: {title}\n\n"
        f"Abstract:\n{abstract}\n\n"
        "Retrieved snippets:\n"
        f"{bank}\n\n"
        "Fill the following JSON scaffold with values grounded in the retrieved snippets. "
...
def main():
    ...
    rows = []
    for item in index:
        full_artifact = load_json(Path(item["artifact_path"]))
        artifact = minimal_artifact(full_artifact)
    ...
    def build(records, train_split=False):
        ...
        bench = []
        for row in records:
            prompt = retrieval_scaffold_prompt(row["title"], row["abstract"], row["scaffold"], row["full_artifact"])
