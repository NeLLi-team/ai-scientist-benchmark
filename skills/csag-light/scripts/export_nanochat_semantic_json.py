...
def base_context(title: str, abstract: str) -> str:
    return f"Paper title: {title}\n\nAbstract:\n{abstract}\n\n"


def full_prompt(title: str, abstract: str) -> str:
    return base_context(title, abstract) + (
        "Return exactly one valid compact csag-light JSON object. "
        "Do not add prose before or after the JSON."
    )


def field_prompts(title: str, abstract: str):
    ctx = base_context(title, abstract)
    return {
        "header": ctx + 'Return only {"paper_id":"...","title":"..."} as JSON.',
        "entities": ctx + 'Return only the "entities" array as JSON.',
        "hypotheses": ctx + 'Return only the "hypotheses" array as JSON.',
        "evidence_items": ctx + 'Return only the "evidence_items" array as JSON.',
        "evidence_links": ctx + 'Return only the "evidence_links" array as JSON.',
        "discoveries": ctx + 'Return only the "discoveries" array as JSON.',
        "gaps": ctx + 'Return only the "gaps" array as JSON.',
        "conclusions": ctx + 'Return only the "conclusions" array as JSON.',
    }
...
def main():
    ...
    def build(records, train_split=False):
        ...
        for row in records:
            ...
            prompts = field_prompts(title, abstract)
            target_full = json.dumps(artifact, ensure_ascii=True, separators=(",", ":"))

            # always include full-object extraction
            convs.append([
                {"role": "user", "content": full_prompt(title, abstract)},
                {"role": "assistant", "content": target_full},
            ])
            if train_split:
                for field_name, prompt in prompts.items():
                    if field_name == "header":
                        target = json.dumps(
                            {"paper_id": artifact["paper_id"], "title": artifact["title"]},
                            ensure_ascii=True,
                            separators=(",", ":"),
                        )
                    else:
                        target = json.dumps(artifact[field_name], ensure_ascii=True, separators=(",", ":"))
                    convs.append([
            ...
            benchmark.append({
                ...
                "prompt": full_prompt(title, abstract),
                "answer": target_full,
            })
        return convs, benchmark
